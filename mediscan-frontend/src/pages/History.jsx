...
import { listHistoryRecords } from "../api/historyApi";
...
export default function History() {
  ...
  useEffect(() => {
    const loadHistory = async () => {
      const localSaved = JSON.parse(localStorage.getItem("scanHistory")) || [];
      try {
        ...
        const mappedApiItems = apiItems.map((item) => ({
          ...
          doctor: item.notes || "N/A",
          status: "Saved",
        }));
        setRecords([...mappedApiItems, ...localSaved]);
      } catch {
        setRecords(localSaved.reverse());
      }
    };
    loadHistory();
  }, []);
